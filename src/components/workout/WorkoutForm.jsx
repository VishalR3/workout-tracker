  ...
  return (
    <form onSubmit={handleSubmit(submitFunction)}>
      <Grid container flexDirection={'column'} spacing={2}>
        ...
        <Grid item>
          <TextField type="text" multiline rows={4} label="Description" {...register("description")} fullWidth InputLabelProps={{
            shrink: isEditForm ? true : undefined,
          }} />
        </Grid>
        <Grid item>
          ...
          {Array(noOfSections + 1).fill(1).map((_, index) => (
            <Grid container key={index} spacing={1} mt={1} flexDirection={'column'}>
              <Grid item>
                <TextField type="text" label="Section Name" {...register(`sections.${index}.name`)} fullWidth InputLabelProps={{
                  shrink: isEditForm ? true : undefined,
                }} />
              </Grid>
              <Grid item>
                <TextField type="text" multiline label="Section Description" {...register(`sections.${index}.description`)} fullWidth InputLabelProps={{
                  shrink: isEditForm ? true : undefined,
                }} />
              </Grid>
              <Grid item>
        ...
        <Grid item>
          <Button type="submit" variant={"contained"} fullWidth>{isEditForm ? "Edit" : "Add"} Workout</Button>
        </Grid>
      </Grid>
    </form>
  )
